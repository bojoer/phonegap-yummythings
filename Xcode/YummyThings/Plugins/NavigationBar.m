...
//
//  Customised and adopted by Saul Zukauskas @sauliuz
...
-(UIBarButtonItem*)backgroundButtonFromImage:(NSString*)imageName title:(NSString*)title fixedMarginLeft:(float)fixedMarginLeft fixedMarginRight:(float)fixedMarginRight target:(id)target action:(SEL)action
{
    ...
    backButton.titleLabel.textAlignment = NSTextAlignmentCenter; //UITextAlignmentCenter;
...
{
    // If Navigation Bar is already created stop.
    if(navBar) {
       return; 
    }
         
    navBarController = [[NavigationBarController alloc] init];
